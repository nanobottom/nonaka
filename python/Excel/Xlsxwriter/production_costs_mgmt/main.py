...
sheet_name = str(start_year) + '年' + str(start_month) + '月'
...
# 列の固定
ws.freeze_panes(4 + project_num, main_column_num) 

date = datetime.datetime(start_year, start_month, 1)
column_count = main_column_num + 1
total_days = 0
for i_month in range(start_month, start_month + end_month_interval):
    move_up_year = i_month // 12
    if move_up_year > 0 and i_month % 12 != 0:
        i_month -= move_up_year * 12
    (_, last_day) = calendar.monthrange(start_year + move_up_year, i_month)
    ...
    for i_day in range(1,last_day + 1):
        ...
        i1.edit_fill(fgColor = '#FF8856')
        i1.edit_height_width(width = 4)
        i1.edit_alignment()

        #土曜日なら青、日曜日、祝日なら赤、それ以外は黄色にして日付と曜日を表示させる
        ans_color = is_sat_sun_holiday(date, '#FFFFBB')
        ...
        for i_row in range(4, white_row_num + project_num + 5):
            ...
            numerical_formula_sumif = '=SUMIF(A' + str(5 + project_num) +':A' + str(white_row_num + project_num + 4) + ',' + str(i_row - 4) + ',' + get_column_letter(column_count) + str(5 + project_num) + ':'+ get_column_letter(column_count) + str(white_row_num + project_num + 4) + ')'
            numerical_formula = '=SUM(' + get_column_letter(column_count) + str(5 + project_num) + ':' + get_column_letter(column_count) + str(5 + project_num + white_row_num - 1) + ')'
            if i_row == 4 and column_count >= 9:
                white_cell = EditCell(wb, ws, column_count, i_row, numerical_formula)
            elif column_count >= 9 and i_row in range(5, project_num + 5):
                white_cell = EditCell(wb, ws, column_count, i_row, numerical_formula_sumif)
            else:
...
for i_project in range(1, project_num + 1):
    ...
    p_title_cell.edit_fill(fgColor = '#9BF9CC')
...
numerical_formula = '=SUM(G' + str(5 + project_num) + ':G' + str(4 + project_num + white_row_num)+')'
